                                             ...
                                             time_t current_time, PropertyVariant val)
{
    ...
    if (std::find(std::begin(errorsList1), std::end(errorsList1), infoID) != std::end(errorsList1))
    {
        ...
        {
            SetBookOfErrorsRecord(paramID, sectionID, componentID, paramClass, "Error Counter", current_time);
        }
        
    }

    if (std::find(std::begin(errorsList2), std::end(errorsList2), infoID) != std::end(errorsList2))
    {
    ...
    if (std::find(std::begin(healthFields), std::end(healthFields), infoID) != std::end(healthFields))
    {
        if (auto ptr (std::get_if<std::string>(&val)); ptr) {
            ...
            {
                SetBookOfErrorsRecord(paramID, sectionID, componentID, paramClass, "Health Related Fault", current_time);
            }
