...
struct fdr_sample_ext
{ // Extended version of the fdr_sample proto message
...
class Record
{
  ...
  public:
    ...
    fdr_sample_ext last_stored_data; // Last fetched value. Used to determine if
                                     // anything changed

    Record(Profile_t& profile, Section_t& section, Component_t& component,
           std::shared_ptr<FDRStore>& fdrLogWriter,
           std::shared_ptr<FDRStore>& fdrStatWriter, InfoGroup_t& infogroup,
           Info_t& info);

    ~Record();

    void Refresh(bool viaTimerSkipChecks); // Update the record with fresh info
                                           // from platform
    ...
    // release the existing store pointer[will call the store object's
    // destructor]
    ...
    inline void
        ResetLogStatStorePtrs(std::shared_ptr<FDRStore>& fdrNewLogWriter,
