...
HttpException::HttpException(long code, std::string message) : code(code)
{
    ...
    this->message_ = "HttpException Code: " + std::to_string(this->code) +
                     ", Message: " + message;
}
...
HttpResponse
    HttpClient::request(const HttpRequestType& req_type, const std::string& url,
                        const std::map<std::string, std::string>* headers,
                        const std::string* payload)
{
    // The curl pointer is created per request, so that it can be safely called
    // by multiple threads Alternatively, it can be re-used per HttpClient
    // instance, but that would need some constrains, either add a locking, or
    // each thread should create their own HttpClient instance.
    CURL* curl;
    CURLcode res;
