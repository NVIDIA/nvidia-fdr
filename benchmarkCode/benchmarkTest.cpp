...
/*void execTrans(sqlite3* DB, int index){
    ...
    std::string sql = "CREATE TABLE TESTTAB" + std::to_string(index) + " (ID INT
PRIMARY KEY NOT NULL,NAME TEXT NOT NULL,EXTRA TEXT);";
    sqlite3_exec(DB,sql.c_str(),NULL,0,NULL);

    int numIns = 10;
    for(int i = 0 ; i < numIns ; i++){
        sql = "INSERT INTO TESTTAB" + std::to_string(index) + " VALUES (" +
std::to_string(i) + ",'n1','ab')"; sqlite3_exec(DB,sql.c_str(),NULL,0,NULL);
    }
...
void execTrans2(sqlite3* DB, int index){
    ...
    for(int i = 0 ; i < numIns ; i++){
        //sql += "(" + std::to_string(i+index*numIns) + ",'n1','" + LONG_STRING
+ "')"  + (i == numIns - 1 ? "" : ", ");
    }
...
/*
    numTrans delay measureUsage textSize insPerTrans
...
int main(int argc, char** argv)
{
    ...
    std::cout << numTransac << " " << transDelay << " " << measureUsage << " "
              << textSize << " " << insPerTrans << std::endl;
    ...
    char* zErrMsg = 0;
    int numTab = 2;
    const std::string sqlEnd =
        " (TIMESTAMP INT NOT NULL,PARAMTYPE TEXT NOT NULL, PARAMNAME TEXT, VALUE INT);";
    ...
    std::cout << std::endl
              << "Total time for " << numTab << " table creations : "
              << std::chrono::duration<double>(end - start).count() << "s"
              << std::endl;

    dispTime(std::chrono::duration<double>(end - start) / numTab,
             "Average Table Creation time");
    ...
    //std::cout << std::endl << "CPU Usage : " << getCurrentValue() << "%" <<
    std::endl;

    std::cout<< std::endl << "Total time for " << numQueries << " inserts : " <<
    std::chrono::duration<double>(end - start).count() <<"s" << std::endl;

    dispTime(std::chrono::duration<double>(end - start) / numQueries, "Insert
    time");
    ...
    std::cout<< std::endl << "Total time for " << numDels << " deletes : " <<
    std::chrono::duration<double>(end - start).count() <<"s"<< std::endl;

    dispTime(std::chrono::duration<double>(end - start) / numQueries,
    "Individual delete time");
    ...
    dispTime(std::chrono::duration<double>(end - start) / numQueries, "Group
    delete time");
    ...
    */
    ...
    if (type == 0)
    {
        ...
        for (int i = 0; i < numTrans; i++)
        {
            ...
            {
                sql += "(" + std::to_string(j + i * numIns) + midString +
                       (j == numIns - 1 ? "" : ", ");
            }
            sqlite3_exec(DB, sql.c_str(), NULL, 0, NULL);
    ...
    else if (type == 1)
    {
        ...
        for (int i = 0; i < numTrans; i++)
        {
            ...
            op << temp.timestamp << " " << temp.paramType << " "
               << temp.paramName << " " << temp.value << std::endl;
            // op.write((char *) &temp, sizeof(sample));
            // std::cout << "OP:" << temp.a << " " << temp.b << " " << temp.c <<
            // " " << temp.d << std::endl;
            usleep(delayus);
        }
        op.close();
    ...
    else if (type == 2)
    {
        ...
        for (int i = 0; i < numTrans; i++)
        {
            ...
            // std::cout << "OP:" << temp1.id() << " " << temp1.text() << " " <<
            // temp1.extra() << " " << temp1.ex() << std::endl;
            usleep(delayus);
        }
        op.close();
    }
    ...
    /*std::ifstream op("testProto.bin", std::ios_base::in |
    std::ios_base::binary); test::sample2 temp2; for(int i = 0 ; i < numTrans ;
    i++){
        //op >> temp.a >> temp.b >> temp.c >> temp.d;
        if(!temp2.ParseFromIstream(&op))
            std::cout <<"Error reading" << std::endl;
        std::cout << "OP:" << temp2.id() << " " << temp2.text() << " " <<
    temp2.extra() << " " << temp2.ex() << std::endl;
    }
    op.close();*/
    ...
    std::cout << "Transactions per second : "
              << numTrans / std::chrono::duration<double>(end - start).count()
              << std::endl;
    if (measureUsage)
    {
